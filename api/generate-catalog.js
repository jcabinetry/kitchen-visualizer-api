...
function buildCatalogPromptV3(body, hasMainReference, hasBaseReference, hasDoorReference, hasDrawerReference) {
  ...
  const drawerReference = hasDrawerReference
    ...
    : doorReference;
  const drawerAttachmentGuide = hasDrawerReference
...
${drawerAttachmentGuide} They control color or material only and must not change door or drawer geometry.

Use the uploaded kitchen photo as the base image and create a photorealistic cabinet refacing preview, not a redesign.

Replace every existing cabinet door with the exact door shown in ${doorReference}. Copy only the geometry actually visible in that reference: its outer edge, frame or slab construction, rail and stile widths when present, center-panel shape when present, bevels, steps, grooves, relief, proportions, and shadow lines. If the reference is a slab door, keep it a slab. If it is a five-piece, raised-panel, recessed-panel, or other profiled door, reproduce that construction and dimensional depth exactly. Do not add details that are absent from the door reference and do not simplify details that are present. Fit the same reference design naturally to every existing cabinet-door size, including double doors, narrow doors, tall pantry doors, and island doors.
