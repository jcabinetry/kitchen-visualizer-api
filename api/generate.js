...
async function redisIncr(key) {
  const redis = getRedisConfig();
  const response = await fetch(`${redis.url}/incr/${encodeURIComponent(key)}`, {
    method: "POST",
    headers: { Authorization: `Bearer ${redis.token}` }
  });
  const data = await response.json();
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const kitchenBase64 = stripDataUrl(body.image);
    if (!kitchenBase64) return res.status(400).json({ error: "Invalid kitchen image." });
    ...
    if (usedNow >= safeMonthlyLimit) {
      ...
      }).catch(err => console.log("Limit email failed:", err?.message || err));
