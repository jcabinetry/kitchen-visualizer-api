...
function compactItems(items){return uniqueItems(items).map(item=>({type:item.type,name:item.name,description:item.description,pdf:item.pdf,page:item.page,imageHint:item.imageHint,bbox:item.bbox}))}
...
function attachAssets(lines,items){const withAssets=uniqueItems(items).filter(item=>itemAsset(item));lines.forEach(line=>{(line.doors||[]).forEach(door=>{const found=withAssets.find(item=>String(item.type||"").toLowerCase().includes("door")&&matchesLabel(door.label,item))||withAssets.find(item=>matchesLabel(door.label,item));if(found){door.image=itemAsset(found);door.thumbnail=itemAsset(found);door.imageStatus="ready";}});(line.finishes||[]).forEach(finish=>{const found=withAssets.find(item=>/(finish|color|colour|swatch|stain|paint)/i.test(String(item.type||""))&&matchesLabel(finish.label,item))||withAssets.find(item=>matchesLabel(finish.label,item));if(found){finish.swatch=itemAsset(found);finish.image=itemAsset(found);finish.thumbnail=itemAsset(found);finish.swatchStatus="ready";}})});return lines}
function imagePlanFromItems(items){return uniqueItems(items).filter(item=>item.imageHint||item.bbox).map(item=>({type:item.type||"item",name:item.name||"",pdf:item.pdf||"",page:item.page||"",hint:item.imageHint||"",bbox:item.bbox||null}))}
...
async function scanPages(name,pages){
  ...
  return uniqueItems((Array.isArray(result.items)?result.items:[]).map(item=>({type:String(item.type||"other"),name:String(item.name||"").trim(),description:String(item.description||""),pdf:String(item.pdf||item.fileName||""),page:String(item.page||""),imageHint:String(item.imageHint||item.hint||""),bbox:Array.isArray(item.bbox)?item.bbox.map(Number).slice(0,4):null})).filter(item=>item.name));
}

async function buildCatalog(name,items){
  const text=JSON.stringify(compactItems(items)).slice(0,150000);
  const content=[{type:"input_text",text:`Convert these extracted cabinet catalog page items into a saved cabinet catalog. Return JSON only: {"lines":[{"name":"collection","description":"","doors":[{"name":"door style","description":"","image":"","imageStatus":"needs-image"}],"finishes":[{"name":"finish/color","description":"","swatch":"","swatchStatus":"needs-image"}],"species":["species"]}],"imagePlan":[{"type":"door or swatch","name":"item name","pdf":"file name","page":"page number","hint":"where the picture/swatch appears","bbox":[x,y,width,height]}],"notes":""}. Put door style names under doors, color/paint/stain/finish names under finishes, and wood/material names under species. Do not invent names, but do not leave it empty if items exist. Manufacturer: ${name}. Items: ${text}`}];
...
export default async function handler(req,res){
  ...
  try{
    ...
    let extracted,lines,method="batched-page-vision",pageImages=Number(body.pageImages||0),sourceItems=inputItems;
    if(inputItems.length){
      extracted=await buildCatalog(name,inputItems);
      lines=normalize(name,extracted);
      lines=attachAssets(lines,inputItems);
